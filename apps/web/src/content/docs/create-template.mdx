...
If you want to deploy a preview of your template, I don't recommend using CI/CD (just for the preview — for your actual portfolio it's perfectly fine). Why not? Because CI/CD typically uses GitHub Actions to deploy, and if someone else uses your template, that Action would run forcing them to use your same hosting provider.
...
Use a manual deployment tool like Wrangler. Is it more work? Yes, but I've prepared two scripts to help.
...
3. Add the `deploy` command to your `package.json`:
...
If you want your template to appear on [devsync.work](https://devsync.work), you can make a pull request to this repository and add it to the template list.
