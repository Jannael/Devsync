...
4. Add the `devsync` configuration to your `package.json`:
   ...
   This field is optional: if your template doesn't compile its own CV, the CLI generates a default CV from `DEVSYNC.json` instead, writing the PDF to the same output path (`public/{name}-{lang}.pdf`).
