...
If `devsync.pathToCompiledCV` is set in `package.json`:
...
If the field is not set, the CLI generates the CV from `DEVSYNC.json` — verify that `DEVSYNC.json` is valid and contains at least one language key.
